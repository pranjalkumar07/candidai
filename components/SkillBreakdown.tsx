...
interface SkillBreakdownProps {
  skills: SkillItem[] | Record<string, number> | null | undefined;
}
...
const getCategoryIcon = (name: string) => {
  ...
  if (lower.includes("comm") || lower.includes("clarity")) {
    return MessageSquare;
  }
  if (lower.includes("confid") || lower.includes("delivery")) {
    return Shield;
  }
...
export default function SkillBreakdown({
  ...
  const normalizedSkills: SkillItem[] = Array.isArray(skills)
    ? skills
  ...
  if (normalizedSkills.length === 0) {
    return (
      <div className="flex flex-col items-center justify-center text-center p-6 sm:p-8 border border-dashed border-[rgba(110,120,180,0.22)] rounded-[12px] bg-[rgba(11,18,36,0.5)] min-h-[190px] gap-2">
        <p className="text-xs font-semibold text-[#F5F7FF]">
          Competency analysis
        </p>
        ...
        return (
          <div key={item.name} className="flex flex-col gap-1.5">
            <div className="flex items-center justify-between text-xs">
              ...
              <div className="flex items-center gap-2">
                <span className="font-bold text-[#F5F7FF]">
                  {item.score}%
                </span>
