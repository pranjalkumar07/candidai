...
export default function RecentInterviewRow({
  ...
  return (
    <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3.5 p-3.5 sm:p-4 rounded-[12px] bg-[rgba(12,19,38,0.7)] border border-[rgba(110,120,180,0.18)] hover:border-[rgba(115,90,255,0.35)] hover:bg-[rgba(26,36,68,0.6)] transition-all duration-200 group shadow-sm">
      {/* Left: Icon & Role & Level */}
      <div className="flex items-center gap-3.5">
        ...
        <div>
          <h4 className="text-sm font-semibold text-[#F5F7FF] group-hover:text-white transition-colors capitalize">
            {role}
          </h4>
      ...
      <div className="flex items-center justify-between sm:justify-end gap-4 sm:gap-6 pl-12 sm:pl-0">
        {/* Type Badge */}
        <span
          className={`text-[11px] font-semibold px-2.5 py-0.5 rounded-[5px] border capitalize ${typeBadgeStyle}`}
        >
          {type || "Technical"}
        </span>
        ...
        {/* Date */}
        <span className="text-xs text-[#69748D] min-w-[85px] text-right hidden sm:inline">
          {formattedDate}
        </span>
        ...
        <Link
          href={targetHref}
          className="inline-flex items-center gap-1 text-xs font-semibold text-[#845CFF] group-hover:text-white transition-colors"
        >
